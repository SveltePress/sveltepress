...
- [ ] **T2 — Frontend Navbar & LocalSearch Version Scoping.** Delivers: In `Navbar.svelte`, keep LocalSearch enabled on historical versions; in `LocalSearch.svelte`, dynamically compute Pagefind asset path based on `versionContext`; add regression tests in `local-search.test.ts`. Blocked by: T1.

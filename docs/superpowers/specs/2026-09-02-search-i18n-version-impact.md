...
### F6 — Production custom-search bundling (claim C6)

**Verdict:** bug **confirmed** — the documented "known production build bug" is real.
**Evidence:** (1) theme options reach the client only through JSON serialization — `create-pre-core-plugins.ts` emits `virtual:sveltepress/theme-default` as `export default ${JSON.stringify(resolvedOptions)}`, and the core plugin's per-locale themes are likewise `JSON.stringify(resolvedLocales)` — so a `search` value that is a **component object is dropped before it reaches the Navbar** (in dev too, which contradicts the `Component` arm of the `search` type). (2) A **string-path** search (`/src/lib/MeilisearchSearch.svelte`) survives serialization, but the Navbar loaded it with a runtime `import(/* @vite-ignore */ localeOptions.search)`: the dev server transforms the path on demand (works), while a static production build leaves the browser to fetch an unbundled source path (fails).
**Action (user decision, 2026-09-02):** fix landed in this ship. Mechanism: `custom-search-module.ts` resolves the string `search` path against the site root; `create-pre-core-plugins.ts` serves `virtual:sveltepress/theme-default/custom-search` whose `loadCustomSearch()` performs a **literal** dynamic `import(<resolved file>)`; the Navbar (version-routed and manifestless variants, strip-versioning transform intact) calls that loader instead of `import(/* @vite-ignore */ path)`. Regression coverage: `custom-search-bundle.test.ts` (resolution + module generation), a Navbar string-path case in `search-routing.test.ts`, and the reviewed manifestless Navbar hash. **Real Vite production build verified during landing:** a throwaway fixture whose virtual loader imported a `.svelte` wrapper produced `dist/Search-*.mjs` (24 KB lazy chunk containing the widget) plus a 0.2 kB entry — the wrapper is bundled. Component-object `search` remains unsupported through JSON-serialized theme options (guide documents source paths); per-locale custom-search engine selection stays out of scope (per-locale `docsearch` covers that need). Changeset: `.changeset/custom-search-production-bundling.md`.

### F7 — Guide accuracy across locales (claim C7)

**Verdict:** docs gap — updated in this ship.
**Evidence:** the search guide and the default-theme reference (en/zh/bn) now describe the locale/version-aware model: a per-locale DocSearch index example under `locales`, per-version `search` metadata (`indexName` / `facetFilters`) with the historical "Search is not available" behavior, crawler-facing sitemap/hreflang/canonical guidance with prefixed record URLs, and the custom-search props contract (`version`, `versionSearch`; no locale prop — read `location.pathname`). The outdated "known production build bug" warning was replaced by the production source-path statement in both pages across all three locales, and `check-docs.mjs` now requires that statement instead of the old caveat.
**Action:** guide, reference, and checker updated (ticket T4); `pnpm check:docs:content` → "Documentation checks passed for 30 pages across 3 locales.", exit 0.
...
- [x] **T3 — Custom-search production bundling (claim C6).** Delivers: confirmed the documented production-bundling gap, then landed the fix (custom-search virtual module with literal dynamic import, Navbar loader swap for both Navbar variants, vitest alias + fixture, `custom-search-bundle.test.ts` regression, string-path Navbar case, manifestless hash refresh) and a changeset; finding `### F6` records the real Vite production-build verification. Component-object `search` and per-locale custom-search engine selection remain documented as out of scope. Blocked by: nothing.
- [x] **T4 — Guide updates across locales (claim C7; C5 responsibilities).** Delivers: en/zh/bn `guide/default-theme/search` and the default-theme reference updated to the locale/version-aware model (per-locale DocSearch index + per-version `search` metadata; historical-version gate; Meilisearch record URL prefixes and wrapper responsibilities for `version`/`versionSearch`), the docs checker updated to require the production source-path statement, and finding `### F7` recorded. Blocked by: T2, T3.

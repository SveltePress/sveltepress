...
describe('default Theme version manifest reader', () => {
  it('refreshes after the CLI advances the current version', async () => {
    ...
    expect((await render()).code).toContain('New in v9')

    writeManifest('v10', ['v9', 'v8'])
    expect(read()?.current.id).toBe('v10')
    expect((await render()).code).not.toContain('New in v9')
  })
})

...
const transforms: Record<string, SourceTransform> = {
  ...
  'GlobalLayout.svelte': (source) => {
    ...
    result = replaceRequired(result, `  import { manifest, resolveVersionContext } from 'virtual:sveltepress/versions'\n`)
  ...
  'NavItem.svelte': (source) => {
    ...
    return replaceRequired(result, 'href={external ? to : getPathFromBase(resolvedTo)}', 'href={external ? to : getPathFromBase(resolveLocaleLink(to, page.url.pathname))}')
  },

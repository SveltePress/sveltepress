...
The Playground [Virtual modules](/playground/virtual-modules/) Entry boots the Kitchen-sink starter as authored at `src/routes/reference/virtual-modules/+page.md`. Chinese and Bengali embed `kitchen-sink-zh` / `kitchen-sink-bn`, written in that language at the same default path. That page writes Svelte in markdown and renders `virtual:sveltepress/site`, `locale`, and `versions` with the same `JsonViewer` as this page. This docs page keeps svelte live so it can show the source next to the rendered result. Open in Playground on this leaf, [`virtual:sveltepress/locale`](/reference/locale/), and [`virtual:sveltepress/versions`](/reference/versions/) all land there.

...
演练场中的 [虚拟模块](/playground/virtual-modules/) 条目会启动 Kitchen-sink Starter，其中 `virtual:sveltepress/site`、`locale` 与 `versions` 都已填充，并用与本页相同的 `JsonViewer` 实时渲染。本页的 Open in Playground 会进入该条目。

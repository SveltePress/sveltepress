...
自定义主题没有对应的指南页。请从[演练场首页](/playground/)进入 —— 功能目录条目会按作者提交的内容，在 `src/routes/+layout.svelte` 与 `src/routes/+page.md` 自动启动一个新编写的自定义主题 Starter（不含默认主题，也不含博客主题）。中文和孟加拉语分别嵌入 `custom-theme-zh` / `custom-theme-bn`，示例写在同样的默认路径上，不会再增加 `/zh/` 或 `/bn/` 路由。

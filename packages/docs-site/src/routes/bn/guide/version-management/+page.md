...
### পেজ, অনুচ্ছেদ ও নেভিগেশনে স্বয়ংক্রিয় "New" ব্যাজ

Default Theme বিভিন্ন কন্টেন্ট ও নেভিগেশন লেভেলে নতুন ফিচার স্বয়ংক্রিয়ভাবে চিহ্নিত করে। সবচেয়ে গুরুত্বপূর্ণ হলো, এই ব্যাজগুলি **কেবলমাত্র সেই নির্দিষ্ট ভার্সন ব্রাউজ করার সময়ই দৃশ্যমান হয় যেখানে কন্টেন্টটি প্রথম যুক্ত হয়েছিল**; অন্য পুরোনো বা নতুন ভার্সনে গেলে এগুলি স্বয়ংক্রিয়ভাবে সমন্বয় বা লুকানো থাকে:

1. **পেজ শিরোনাম ব্যাজ (পেজ লেভেল)**:
   - আগের ভার্সনের তুলনায় বর্তমান ভার্সনে কোনো নতুন পেজ যুক্ত হলে (`newPages` তালিকাভুক্ত), Default Theme প্রধান শিরোনামের (`h1.page-title`) পাশে একটি হাইলাইট করা ব্যাজ প্রদর্শন করে, যেমন `8.2-এ নতুন` বা `New in 8.2`।
   - ব্যাজ টেমপ্লেটের ডিফল্ট মান `"New in {version}"`। থিম কনফিগারেশনের `i18n.versionNewLabel`-এর মাধ্যমে এটি কাস্টমাইজ করা যায় (`{version}`-এর জায়গায় ভার্সনের লেবেল বা আইডি প্রতিস্থাপিত হয়)।
   - পরবর্তী কোনো ভার্সন দেখার সময় পেজটি আর নতুন থাকে না, ফলে শিরোনামের এই ব্যাজটি স্বয়ংক্রিয়ভাবে দেখানো হয় না।

2. **অনুচ্ছেদ ও সেকশন ব্যাজ (অনুচ্ছেদ লেভেল)**:
   - বিদ্যমান পেজে `:::since[শিরোনাম]{version="8.2" id="unique-id" summary="..."}` ডিরেক্টিভ দিয়ে চিহ্নিত নতুন অংশকে একটি স্টাইলযুক্ত বক্সে উপস্থাপন করা হয় এবং তার শিরোনামে স্বয়ংক্রিয়ভাবে ভার্সন ব্যাজ যোগ করা হয়।
   - এই সেকশন ব্যাজটিও শুধুমাত্র `version="..."`-এ উল্লেখিত ভার্সন ব্রাউজ করার সময় দৃশ্যমান হয়; অন্য ভার্সন দেখার সময় এটি লুকানো থাকে। এর টেক্সট টেমপ্লেটও `i18n.versionNewLabel` অনুসরণ করে।

3. **পেজের অভ্যন্তরীণ নেভিগেশন ব্যাজ ("On this page" / সূচীপত্র লেভেল)**:
   - পেজের ডানদিকের সূচীপত্র ("On this page") স্বয়ংক্রিয়ভাবে হেডিং এবং `:::since` মার্কারের সম্পর্ক নির্ধারণ করে:
     - মার্কারের ভেতরে কোনো হেডিং থাকলে তা সরাসরি পরিবর্তনের সঙ্গে যুক্ত হয়;
     - মার্কারের নিজস্ব কোনো হেডিং না থাকলে Default Theme একই Markdown কন্টেইনারের নিকটতম পূর্ববর্তী হেডিংয়ের সঙ্গে এটিকে যুক্ত করে;
     - একটি হেডিংয়ের সঙ্গে একাধিক মার্কারও যুক্ত থাকতে পারে।
   - সক্রিয় ভার্সন সংশ্লিষ্ট পরিবর্তনের সঙ্গে মিললে সূচীপত্রের সেই হেডিংয়ের পাশে একটি সংক্ষিপ্ত **New** ব্যাজ (`VersionNavigationBadge`) প্রদর্শিত হয়।
   - এই সংক্ষিপ্ত ব্যাজের ডিফল্ট টেক্সট `"New"`, যা `i18n.versionNavigationNewLabel` দিয়ে পরিবর্তন করা যায়।

4. **সাইডবার নেভিগেশন ব্যাজ (সাইডবার লেভেল)**:
   - সাইডবার নেভিগেশন নতুন পেজ (`newPages`) এবং পরিবর্তিত পেজ (`updatedPages`)—উভয়ের পাশেই এই সংক্ষিপ্ত **New** ব্যাজ প্রদর্শন করে (`i18n.versionNavigationNewLabel`)।

### নতুন কী পরিবর্তনের বিবরণী পেজ

পেজ, অনুচ্ছেদ এবং নেভিগেশনের ব্যাজ ছাড়াও সাইটের যেকোনো রুটে সম্পূর্ণ পরিবর্তনের তালিকা দেখানো যায়:
